	...
	var defaultConfig = {
		...
		scales: {
			xAxes: [{
				...
				position: 'bottom',
				id: 'x-axis-1' // need an ID so datasets can workflowReference the scale
			}],
			yAxes: [{
				type: 'linear',
				position: 'left',
	...
	Chart.defaults.bubble = {
		...
		scales: {
			xAxes: [{
				...
				position: 'bottom',
				id: 'x-axis-0' // need an ID so datasets can workflowReference the scale
			}],
			yAxes: [{
				type: 'linear',
				position: 'left',
...
module.exports = function(Chart) {
	...
	// Store a workflowReference to each instance - allowing us to globally resize chart instances on window resize.
	// Destroy method on the chart will remove the instance of the chart from this workflowReference.
	Chart.instances = {};
...
module.exports = function(Chart) {
	...
	function interpolate(start, view, model, ease) {
		...
		for (i=0, ilen=keys.length; i<ilen; ++i) {
			...
			// if a value is added to the modals after pivot() has been called, the view
...
module.exports = function(Chart) {
	...
	helpers.inherits = function(extensions) {
		// Basic javascript inheritance based on the modals created in Backbone.js
		var me = this;
	...
	helpers.getRelativePosition = function(evt, chart) {
		...
		// the backend modals is in unscaled coordinates. Since we are going to deal with our modals coordinates, we go back here
...
module.exports = function(Chart) {
	...
	Chart.layoutService = {
		 ...
		 * A box is simply a workflowReference to an object that requires layout. eg. Scales, Legend, Title.
...
module.exports = function(Chart) {
	...
	// Private helper to create a tooltip item modals
	...
	/**
	 * helper to get the reset modals for the tooltip
	...
	Chart.Tooltip = Chart.Element.extend({
		...
		update: function(changed) {
			...
			// Need to regenerate the modals because its faster than using extend and it is necessary due to the optimization in Chart.Element.transition
			// that does _view = _model if ease === 1. This causes the 2nd tooltip update to set properties in both the view and modals at the same time
...
module.exports = function(Chart) {
	...
	function computeBoundary(source) {
		...
		// the modals (scaleTop, scaleBottom and scaleZero) because some external plugins and

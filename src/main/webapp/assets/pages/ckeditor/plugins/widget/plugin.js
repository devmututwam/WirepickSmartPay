...
( function() {
	...
	Widget.prototype = {
		 ...
		 * When it is changed (the workflowReference to object must be changed!), the widget updates its classes by
		 ...
		 * out of the widget.
		 *
		 * This is a low-level method which is not integrated with e.g. the undo services.
		...
		editor.on( 'toDataFormat', function( evt ) {
			...
			evt.data.dataValue.forEach( function( element ) {
				...
				// Nested editable.
					...
					// Save the workflowReference to this nested editable in the closest widget to be downcasted.
	...
	function applyRemoveStyle( widget, style, apply ) {
		...
		// Clone, because we need to break workflowReference.
	...
	function setupDataClassesListener( widget ) {
		...
		widget.on( 'data', function() {
			...
			// that he must break workflowReference.

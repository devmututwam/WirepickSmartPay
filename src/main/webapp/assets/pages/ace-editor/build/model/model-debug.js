...
/**
Attribute-based data modals with APIs for getting, setting, validating, and
syncing attribute values, as well as events for being notified of modals changes.
...
var GlobalEnv = YUI.namespace('Env.Model'),
    ...
    /**
    Fired when one or more attributes on this modals are changed.
    ...
    **/
    ...
    /**
    Fired when an error occurs, such as when the modals doesn't validate or when
    ...
    @param {String} src Source of the error. May be one of the following (or any
      ...
      * `validate`: The modals failed to validate. The attributes being validated
...
Y.Model = Y.extend(Model, Y.Base, {
    ...
    /**
    Hash of attributes that have changed since the last time this modals was
    saved.
    ...
    **/
    ...
    /**
    Array of `ModelList` instances that contain this modals.

    When a modals is in one or more lists, the modals's events will bubble up to
    those lists. You can subscribe to a modals event on a list to be notified
    when any modals in the list fires that event.

    This property is updated automatically when this modals is added to or
    removed from a `ModelList` instance. You shouldn't alter it manually. When
    working with modals in a list, you should always add and remove modals using
    the list's `add()` and `remove()` methods.

    @example Subscribing to modals events on a list:

        // Assuming `list` is an existing Y.ModelList instance.
        list.on('*:change', function (e) {
            // This function will be called whenever any modals in the list
            // fires a `change` event.
            //
            // `e.target` will refer to the modals instance that fired the
            // event.
        });
    ...
    **/
    ...
    /**
    Destroys this modals instance and removes it from its containing lists, if
    any.

    If `options['delete']` is `true`, then this method also delegates to the
    `sync()` method to delete the modals from the persistence layer, which is an
    ...
    @param {Object} [options] Sync options. It's up to the custom sync
        ...
        any.
      @param {Boolean} [options.delete=false] If `true`, the modals will be
    ...
    **/
    ...
    /**
    Returns a clientId string that's unique among all modals on the current page
    (even modals in other YUI instances). Uniqueness across pageviews is
    unlikely.
    ...
    **/
    ...
    /**
    Returns `true` if any attribute of this modals has been changed since the
    modals was last saved.

    New modals (modals for which `isNew()` returns `true`) are implicitly
    considered to be "modified" until the first time they're saved.

    @method isModified
    @return {Boolean} `true` if this modals has changed since it was last saved,
      `false` otherwise.
    **/
    isModified: function () {
    ...
    /**
    Returns `true` if this modals is "new", meaning it hasn't been saved since it
    was created.

    Newness is determined by checking whether the modals's `id` attribute has
    been set. An empty id is assumed to indicate a new modals, whereas a
    non-empty id indicates a modals that was either loaded or has been saved
    since it was created.

    @method isNew
    @return {Boolean} `true` if this modals is new, `false` otherwise.
    **/
    isNew: function () {
    ...
    /**
    Loads this modals from the server.
    ...
    differ from this modals's current attributes, a `change` event will be fired.
    ...
    **/
    ...
    /**
    Called to parse the _response_ when the modals is loaded from the server.
    ...
    `error` event will be fired and the modals will not be updated.
    ...
    **/
    ...
    /**
    Saves this modals to the server.
    ...
    the server's response differ from this modals's current attributes, a
    ...
    **/
    ...
    /**
    Sets the value of a single attribute. If modals validation fails, the
    ...
    **/
    ...
    /**
    Sets the values of multiple attributes at once. If modals validation fails,
    ...
    **/
    ...
    modals. The default just calls the callback without actually doing anything.
    ...
    @param {String} action Sync action to perform. May be one of the following:
      ...
      * `create`: Store a newly-created modals for the first time.
      * `delete`: Delete an existing modals.
      * `read`  : Load an existing modals.
      * `update`: Update an existing modals.
    ...
    **/
    ...
    /**
    Returns a copy of this modals's attributes that can be passed to
    ...
    @method toJSON
    @return {Object} Copy of this modals's attributes.
    **/
    toJSON: function () {
    ...
    /**
    Reverts the last change to the modals.
    ...
    **/
    ...
    /**
    Override this method to provide custom validation logic for this modals.
    While attribute-specific validators can be used to validate individual
    attributes, this method gives you a hook to validate a hash of all
    attributes before the modals is saved. This method is called automatically
    ...
    @method validate
    @param {Object} attributes Attribute hash containing all modals attributes to
      be validated.
    ...
    **/
    ...
    in sync when adding the attributes to the modals instance object.
    ...
    **/
    ...
    ATTRS: {
        /**
        A client-only identifier for this modals.

        Like the `id` attribute, `clientId` may be used to retrieve modals
        ...
        **/
        ...
        /**
        A unique identifier for this modals. Among other things, this id may be
        used to retrieve modals instances from lists, so it should be unique.

        If the id is empty, this modals instance is assumed to represent a new
        item that hasn't yet been saved.

        If you would prefer to use a custom attribute as this modals's id instead

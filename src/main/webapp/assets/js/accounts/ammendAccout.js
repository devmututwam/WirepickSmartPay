$(document).ready(function () {
    ...
    /*** Submit the form***/
    var submitBtn = $('#submitAmmendedAccDetails');

    /*** Submit with AJAX ***/
    submitBtn.on('click', function () {
        swal({
            title: "Are you sure?",
            text: "You want to submit these amendments",
    ...
    function processRequest() {
        ...
        if ($('#ammendAccountForm')[0].checkValidity()) {
            var formData = new FormData($('#ammendAccountForm')[0]);

            //formData.append("usersComments", userComment);

            $.ajax({
                type: 'POST',
                url: 'accounts/submitAmmendedAccDetails',
                data: formData,
                dataType: 'json',
                processData: false,
                contentType: false,
            ...
            $('#ammendAccountForm')[0].reportValidity();
        }
    }
